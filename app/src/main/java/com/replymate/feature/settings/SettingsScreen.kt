...
@Composable fun SettingsScreen(theme: AppTheme, geminiConfigured: Boolean, onThemeChanged: (AppTheme) -> Unit, onPersonalization: () -> Unit, onGeminiSettings: () -> Unit, onPlayground: () -> Unit, onPlatformEvents: () -> Unit, onDiagnostics: () -> Unit, onBack: () -> Unit) {
    Scaffold(topBar = { TopAppBar(title = { Text("Settings") }, navigationIcon = { TextButton(onClick = onBack) { Text("Back") } }) }) { padding ->
        Column(Modifier.padding(padding)) {
            ...
            SettingRow("Developer Diagnostics", "Local performance, integrity, and error statistics", onDiagnostics)
