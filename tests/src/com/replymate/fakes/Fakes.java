...
/** P-background-12: the store fakes are MONITOR-SERIALIZED (every store method
 *  synchronized) so multi-thread generation tests exercise the REAL overlap
 *  (matches the real stores, where SQLite serializes writers) instead of
 *  racing unsynchronized ArrayList/HashMap internals. Single-threaded tests
 *  are behavior-identical — reentrant monitors change no semantics. */
...
public final class Fakes {
    ...
    public static final class ContactStoreFake implements ContactStore {
        ...
        public synchronized void put(Contact c) { byId.put(c.id, c); }
        @Override public synchronized long insert(Contact c) { if (c.id == 0) c.id = nextId++; byId.put(c.id, c); return c.id; }
        @Override public synchronized void update(Contact c) { byId.put(c.id, c); }
        @Override public synchronized Contact get(long contactId) { return byId.get(contactId); }
        @Override public synchronized List<Contact> all() { return new ArrayList<Contact>(byId.values()); }
        @Override public synchronized void delete(long contactId) { byId.remove(contactId); }
        @Override public synchronized List<ContactChannel> channelsByContact(long contactId) {
            List<ContactChannel> out = new ArrayList<ContactChannel>();
            for (ContactChannel ch : channels) if (ch.contactId == contactId) out.add(ch);
            return out;
        }
        @Override public synchronized ContactChannel findChannel(Channel channel, String remoteKey) {
            for (ContactChannel ch : channels) {
                if (ch.channel == channel && ch.remoteKey.equals(remoteKey)) return ch;
            }
            return null;
        }
        @Override public synchronized long upsertChannel(ContactChannel ch) {
        ...
        @Override public synchronized void touchChannel(long channelId, long lastSeenAt) {
        ...
        @Override public synchronized void reassignContact(long fromContactId, long toContactId) {
    ...
    public static final class MessageStoreFake implements MessageStore {
        ...
        @Override public synchronized long insert(Message m) { add(m); return m.id; }
        ...
        @Override public synchronized Message getByNotifKey(Channel channel, String notifKey) {
        ...
        @Override public synchronized Message findRecentSame(long contactId, Channel channel, Direction dir, String body, long ts, long windowMs) {
        ...
        @Override public synchronized List<Message> lastMessages(long contactId, int limit) {
            List<Message> all = byContact.get(contactId);
        ...
        @Override public synchronized List<Message> olderThanId(long contactId, long beforeId, int limit) {
            List<Message> all = byContact.get(contactId);
        ...
        @Override public synchronized List<Message> searchByBody(String query, int limit) {
        ...
        @Override public synchronized void deleteByContact(long contactId) { byContact.remove(contactId); }
    }

    public static final class StyleStoreFake implements StyleStore {
        public StyleProfile global;
        @Override public synchronized StyleProfile get(Scope scope, Long contactId) {
            return scope == Scope.GLOBAL ? global : null;
        }
        @Override public synchronized long upsert(StyleProfile p) { if (p.scope == Scope.GLOBAL) global = p; return 1; }
        @Override public synchronized void deleteByContact(long contactId) { }
    }

    public static final class DraftStoreFake implements DraftStore {
        public final List<Draft> saved = new ArrayList<Draft>();
        private long nextId = 1;
        @Override public synchronized long insert(Draft d) { d.id = nextId++; saved.add(d); return d.id; }
        @Override public synchronized void reassignContact(long fromContactId, long toContactId) {
            for (Draft d : saved) if (d.contactId == fromContactId) d.contactId = toContactId;
        }
        /** Contract honors the port: newest-first. */
        @Override public synchronized List<Draft> byContact(long contactId, int limit) {
        ...
        @Override public synchronized List<Draft> byVariantGroup(String variantGroup) { return saved; }
        @Override public synchronized void updateStatus(long draftId, DraftStatus status) {
            for (Draft d : saved) if (d.id == draftId) d.status = status;
        }
        @Override public synchronized void updateText(long draftId, String newText) {
            for (Draft d : saved) if (d.id == draftId) d.replyText = newText;
        }
        @Override public synchronized void updateFavorite(long draftId, boolean favorite) {
            for (Draft d : saved) if (d.id == draftId) d.favorite = favorite;
        }
        @Override public synchronized void delete(long draftId) {
    ...
    public static final class MemoryStoreFake implements com.replymate.core.ports.MemoryStore {
        ...
        @Override public synchronized List<com.replymate.core.model.MemoryFact> activeFacts(long contactId) {
        ...
        @Override public synchronized List<com.replymate.core.model.MemoryFact> allFacts(long contactId) {
        ...
        @Override public synchronized long upsertFact(com.replymate.core.model.MemoryFact f) {
        ...
        @Override public synchronized void setFactPinned(long factId, boolean pinned) {
        ...
        @Override public synchronized void setFactDisabled(long factId, boolean disabled) {
            for (List<com.replymate.core.model.MemoryFact> l : factsByContact.values()) {
                for (com.replymate.core.model.MemoryFact f : l) if (f.id == factId) f.disabled = disabled;
            }
        }
        @Override public synchronized void deleteFact(long factId) {
            for (List<com.replymate.core.model.MemoryFact> l : factsByContact.values()) {
                for (int i = l.size() - 1; i >= 0; i--) if (l.get(i).id == factId) l.remove(i);
            }
        }
        @Override public synchronized com.replymate.core.model.ContactSummary latestSummary(long contactId) {
        ...
        @Override public synchronized long insertSummary(com.replymate.core.model.ContactSummary s) {
        ...
        @Override public synchronized void deleteAllForContact(long contactId) {
    ...
    public static final class StyleSettingStoreFake implements com.replymate.core.ports.StyleSettingStore {
        ...
        @Override public synchronized Map<String, String> all(Long contactId) {
            return new HashMap<String, String>(scope(contactId));
        }
        @Override public synchronized void put(Long contactId, String key, String value) {
            scope(contactId).put(key, value);
        }
        @Override public synchronized void remove(Long contactId, String key) {
    ...
    public static final class LearningStoreFake implements com.replymate.core.ports.LearningStore {
        ...
        @Override public synchronized long insert(StyleSignal s) {
        ...
        @Override public synchronized List<StyleSignal> byContact(long contactId, int limit) {
        ...
        @Override public synchronized void deleteForContact(long contactId) {
    ...
    public static final class UsageStoreFake implements UsageStore {
        ...
        @Override public synchronized long insert(UsageEvent e) { events.add(e); return events.size(); }
        @Override public synchronized long totalTokensSince(long ts) {
            long t = 0;
            for (UsageEvent e : events) t += e.tokensIn + e.tokensOut;
            return t;
        }
        @Override public synchronized int countSince(long ts) { return events.size(); }
        @Override public synchronized List<UsageEvent> since(long ts) {
            List<UsageEvent> out = new ArrayList<UsageEvent>();
            for (int i = events.size() - 1; i >= 0; i--) if (events.get(i).ts >= ts) out.add(events.get(i));
            return out;
        }
        @Override public synchronized void purgeBefore(long ts) { }
    }

    public static final class KvStoreFake implements KvStore {
        private final Map<String, String> map = new HashMap<String, String>();
        @Override public synchronized String get(String key, String defValue) {
            return map.containsKey(key) ? map.get(key) : defValue;
        }
        @Override public synchronized void put(String key, String value) { map.put(key, value); }
        @Override public synchronized void delete(String key) { map.remove(key); }
    ...
    public static final class FakeProvider implements AiProvider {
        ...
        @Override public synchronized String type() { return "gemini"; }
        @Override public synchronized Result<ChatReply> generate(ChatRequest request) {
            calls++;
            lastRequest = request;
            return result;
        }
        @Override public synchronized Result<Boolean> validateKey() { return Result.ok(Boolean.TRUE); }
        @Override public synchronized Result<java.util.List<String>> listModels() {
    ...
    public static final class GatewayFake implements ProviderGateway {
        ...
        @Override public synchronized AiProvider active() { return provider; }
        @Override public synchronized String activeModel() { return model; }
        @Override public synchronized com.replymate.core.model.ProviderRef activeMeta() { return provider == null ? null : meta; }
    }
    ...
    public static final class ProviderStoreFake implements com.replymate.core.ports.ProviderStore {
        ...
        private long nextId = 1;
        @Override public synchronized long upsertActive(com.replymate.core.model.ProviderDef def) {
        ...
        @Override public synchronized com.replymate.core.model.ProviderDef active() {
            for (com.replymate.core.model.ProviderDef r : rows) if (r.isActive) return r;
            return null;
        }
        @Override public synchronized java.util.List<com.replymate.core.model.ProviderDef> all() {
            return new java.util.ArrayList<com.replymate.core.model.ProviderDef>(rows);
        }
        @Override public synchronized void setActive(long id) {
    ...
    public static final class SecretVaultFake implements com.replymate.core.ports.SecretVault {
        ...
        @Override public synchronized void putSecret(String alias, String value) { map.put(alias, value); }
        @Override public synchronized String getSecret(String alias) { return map.get(alias); }
        @Override public synchronized boolean hasSecret(String alias) { return map.containsKey(alias); }
        @Override public synchronized void deleteSecret(String alias) { map.remove(alias); }
    }
    ...
    public static final IdGen IDS = new IdGen() {
        private int n;
        @Override public synchronized String next() { return "group-" + (++n); }
    };

    public static final Logger NOOP_LOG = new Logger() {
        @Override public synchronized void d(String tag, String msg) { }
        @Override public synchronized void i(String tag, String msg) { }
        @Override public synchronized void w(String tag, String msg) { }
        @Override public synchronized void e(String tag, String msg) { }
        @Override public synchronized void e(String tag, String msg, Throwable t) { }
    };
}
